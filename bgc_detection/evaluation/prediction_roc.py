#!/usr/bin/env python
# David Prihoda
# Plot ROC curve plot from Domain CSV with predictions
...
def prediction_roc_plot(predictions_per_model, names, colors, title, figsize=(5, 5)):
    """
    Plot ROC curve plot from list of domain predictions (Domain DataFrames with prediction column)
...
if __name__ == "__main__":
    ...
    fig = prediction_roc_plot(predictions, options.names, options.colors, options.title, figsize=(options.size, options.size))
    fig.savefig(options.output, dpi=150, bbox_inches='tight')

    print('Saved ROC plot to: ', options.output)

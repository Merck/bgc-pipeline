...
for file in ../../genome/*.fa; do 
    qsub -V -b y -cwd -l mem_reserve=10G -l mem_free=10G -l h_vmem=10G -pe threads 4 \
        antismash $file --minlength 1 --skip-cleanup --inclusive --enable cf_saccharide,cf_fatty_acid \
        --disable-embl --disable-html --disable-svg --disable-xls --logfile $(basename "$file").log --debug;  
done;
```
